"""
    File Name: Tensorflow LSTM Predictions.py
    Date: 8/17/2019
    Author: reed.clarke@rcsoftwareservices.com
"""
# Make sure that you have all these libaries available to run the code successfully
# from data_reader.py import AlphaVantageDataReader
# from pandas_datareader import data
import math
from server.reader.reader import YahooFinanceApiReader
from server.formatter.formatter import YahooFinanceApiFormatter
import sys
import json
...
# dr = AlphaVantageDataReader(api_url_params_query_name='adjusted-full-csv',
#                             absolute_or_relative_csv_file_path='predictor/df_files/alpha-vantage/',
#                             data_source='api', api_data_type='csv')
# df = dr.get_data_from_csv_file('AAL', "TIME_SERIES_DAILY_ADJUSTED")
# if df is None:
#     df = dr.get_data_from_api(append_to_csv_file=False, convert_df=True, download=True)
# dr.fill_in_missing_dates(df)
...
if data_source == 'alphavantage':
    ...
    # If the data is already there, just load it from the CSV
        ...
        print('Reading data from CSV', len(df))
    ...
    df = pd.read_csv(os.path.join("C:/Stock Manager/Resources/Stocks", 'aal.us.txt'),
                     delimiter=',', usecols=['Date', 'Open', 'High', 'Low', 'Close'])
    print('Loaded data from the Kaggle repository', len(df))
...
end_date = '2017-07-28'
reader = YahooFinanceApiReader(save=True)
df = reader.get_historical_data(ticker, start_date, end_date)
df = df.sort_index(ascending=True, axis=0)
df = df.reset_index()
columns = ['date', 'ticker', 'adjclose', 'close', 'high', 'low', 'open', 'volume']
df = df[columns]
df.index = range(len(df))
data_size = len(df)
print("data_size ", data_size)
train_data_size = math.floor(len(df) * .90)
print("train_data_size ", train_data_size)

# sys.exit()

# Double check the result
x_tick_loc = math.floor(data_size * .05)
plt.figure(figsize=(18, 9))
plt.plot(range(df.shape[0]), (df['low'] + df['high']) / 2.0)
plt.xticks(range(0, df.shape[0], x_tick_loc), df['date'].loc[::x_tick_loc], rotation=45)
...
smoothing_window_size = math.floor(train_data_size * .25)
if smoothing_window_size % 2 != 0:
    smoothing_window_size += 1
print("smoothing_window_size = ", smoothing_window_size)
# smoothing_window_size = 750  # math.floor(train_data_size * .25)
# print("smoothing_window_size = ", smoothing_window_size)
for di in range(0, train_data_size, smoothing_window_size):
...
window_size = math.floor(train_data_size * .025)
# window_size = 25  # math.floor(train_data_size * .025)
...
w = tf.compat.v1.get_variable('w', shape=[num_nodes[-1], 1], initializer=tf.contrib.layers.xavier_initializer())
...
# all_outputs = tf.nn.xw_plus_btf.compat.v1.nn.xw_plus_b(all_lstm_outputs, w, b)
